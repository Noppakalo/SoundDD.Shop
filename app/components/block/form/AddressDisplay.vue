...
const customerType = computed(() => {
    return (
        props.customer?.meta_data?.find((m: any) => m.key === 'customer_type')
            ?.value || 'natural'
    )
})
...
const taxId = computed(() => {
    return (
        props.customer?.meta_data?.find((m: any) => m.key === 'tax_id')
            ?.value || ''
    )
})
</script>
